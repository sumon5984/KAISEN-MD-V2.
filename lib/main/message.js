...
async save(m) {
  ...
  const mediaMsg = m.message[type];

  const mime = mediaMsg?.mimetype || '';
  const mediaType = mime.split('/')[0]; // 'image', 'video', etc.
...
class serialize {
	constructor(conn, m, createrS, store) {
		...
		this.owners = [jidNormalizedUser(this.client.user.id), "917439489057@s.whatsapp.net"].concat(this.sudo);
...
async download() {
  const message = this.msg?.msg || this.msg;
  const mime = message?.mimetype || '';
  const mtype = mime.split('/')[0]; // 'image', 'video', etc.
...
async updateProfilePicture(jid, content) {
  // `content` should be a Buffer (e.g. from this.download())
